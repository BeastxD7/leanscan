 ...
 * Local notifications still work in Expo Go on SDK 55, so we can dev-test
 ...
 *   - If denied, schedule calls silently no-op; settings shows a "Blocked"
 *     state with a CTA to open OS settings
 */
 ...
 *
 * Silently no-ops if permission isn't granted (caller is responsible for the
 * permission prompt UX before reaching here).
 */
